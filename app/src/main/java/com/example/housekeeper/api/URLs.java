package com.example.housekeeper.api;

public class URLs {

    public static final String ROOT_URL = "http://192.168.0.106:9021/fofTaskAssignmentApi/";
//    public static final String LOGIN_URL = "http://175.41.47.106/alamafour/api/login";

    public static final String URL_AUTH = ROOT_URL + "auth?";
    public static final String URL_TASKS = ROOT_URL + "tasklist";
    public static final String URL_TASK_UPDATE = ROOT_URL + "statusUpdate";
    public static final String URL_CHECKLIST_TYPE = ROOT_URL + "checkListType";
    public static final String URL_CHECKED_LISTS = ROOT_URL + "checkedList";

}
